$(document).ready(function () {
    ...
    $.each( $('.knob'), function() {
        ...
        if(knobActive.attr('data-type')=='continuous'){
            knobActive.find(".marker_container").css('transform', "rotate("+maxAngle*val/100+"deg)");
        }
        else if( knobActive.attr('data-type') == 'stepped'){
            stepNum = parseInt(knobActive.attr('data-steps'));
            stepLength = (maxAngle-initialAngle)/stepNum;
            knobActive.find(".marker_container").css('transform', "rotate("+initialAngle+stepLength*val+"deg)");
        }
        else  $('#'+ $(this).attr('id') +' .marker_container').css('transform', "rotate("+initialAngle+"deg)");
    });
    ...
    var offset = knobActive.offset();
        ...
        if(angle<initialAngle) angle=initialAngle;
        else if (angle>maxAngle) angle = maxAngle;
        
        if(knobActive.attr('data-type')=='continuous'){
            knobActive.find(".marker_container").css('transform', "rotate("+angle+"deg)");
            knobActive.attr('data-value', (angle-initialAngle)*100/(maxAngle-initialAngle));
        }
        else if( knobActive.attr('data-type') == 'stepped'){
            stepNum = parseInt(knobActive.attr('data-steps'));
            stepLength = (maxAngle-initialAngle)/stepNum;
            currentStep = parseInt(angle/stepLength);
            knobActive.find(".marker_container").css('transform', "rotate("+initialAngle+stepLength*currentStep+"deg)");
