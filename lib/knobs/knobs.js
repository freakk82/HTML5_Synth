$(document).ready(function () {
    ...
    // this works for older web layout engines
    $(".switchCOntainer").onmousedown = function(event) {
                event.preventDefault();
                return false;
              };
              
    $(".switchContainer").mousedown(function() { 
        $(this).children('.switch').addClass('pressed');
    });
    $(".switchContainer").mouseup(function() { 
        $(this).children('.switch').removeClass('pressed');
        $(this).children('.led').toggleClass('active');
        $(this).attr('data-value', parseInt( $(this).attr('data-value') * -1));  // data-value > 0 -->  ON
    });
